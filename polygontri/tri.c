     ...
     char *argv[];
{
  ...
  if ( ((n = read_segments("/Users/TLG/Documents/GitHub/XCodeCppTest/polygontri/data_1", &genus)) < 0))
    {
...
int triangle(vertices, cnt, triangles)
    double (*vertices)[2];
    int cnt;
    int (*triangles)[3];
{
    memset((void *)seg, 0, sizeof(seg));
    initialise(cnt);
    construct_trapezoids(cnt);
    int nmonpoly = monotonate_trapezoids(cnt);
    return triangulate_monotone_polygons(cnt, nmonpoly, triangles);
}
